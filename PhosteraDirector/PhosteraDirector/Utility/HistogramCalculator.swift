...
class HistogramCalculator {
    ...
    private let queue = DispatchQueue(label: "com.phostera.director.camera.histogram")
    var layer:CALayer?
    ...
    func calculate(for sampleBuffer: CMSampleBuffer) {
        queue.async { [weak self] in
            ...
            let scaleFactor = maxCount == 0 ? 0 : Float(layer.bounds.size.height) / Float(maxCount) * 10
            ...
            for y in [0.0, 1.0, 2.0, 3.0] {
                ...
                for x in widths {
                    point.x = x
                    point.y = 200 - CGFloat(Float(self.histogram[(Int(y) * 256) + Int(x)]) * scaleFactor)
