...
public class CameraManager: NSObject {
    ...
    @Published var wantsAudioUUIDs:Set<String> = []
    ...
    func requestScreenshot(uuid:String) {
    ...
    func wantsAudio(uuid:String) -> Bool {
        return self.wantsAudioUUIDs.contains(uuid)
    }
    ...
    func setWantsAudio(uuid:String, audio:Bool) {
        if audio {
            if !self.wantsAudioUUIDs.contains(uuid) {
                self.wantsAudioUUIDs.insert(uuid)
            }
        } else {
            if self.wantsAudioUUIDs.contains(uuid) {
                self.wantsAudioUUIDs.remove(uuid)
            }
        }
    }
    ...
    func loadAvailableCameras() {
        ...
        if itemContents != nil {
            ...
            Logger.shared.info("loadAvailableCameras - none found")
        }
    }
