...
class PreviewMetalView: MTKView {
    ...
    func updatePreferredFPS(rate:Int) {
    ...
    override func draw(_ rect: CGRect) {
        ...
        guard let cvTexture = cvTextureOut, let texture = CVMetalTextureGetTexture(cvTexture) else {
            Logger.shared.error("Failed to create preview texture")
        ...
        guard let commandBuffer = commandQueue.makeCommandBuffer() else {
            Logger.shared.error("Failed to create Metal command buffer")
            CVMetalTextureCacheFlush(textureCache!, 0)
            return
        }
        
        guard let commandEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: currentRenderPassDescriptor) else {
            Logger.shared.error("Failed to create Metal command encoder")
            CVMetalTextureCacheFlush(textureCache!, 0)
            return
        }
