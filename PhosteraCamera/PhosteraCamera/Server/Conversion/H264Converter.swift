...
class H264Converter {
    ...
    private var baseTime:CMTime = CMTime(value: 0, timescale: 30)
    private var duration:CMTime = CMTime(value: 1, timescale: 30)
    ...
    private func createSampleBuffer(with blockBuffer: CMBlockBuffer) -> CMSampleBuffer? {
        ...
        timingInfo.presentationTimeStamp = baseTime
        ...
        guard error == noErr,
            ...
            Logger.shared.error("fail to create sample buffer")
            return nil
        }
        ...
        baseTime = CMTimeAdd(baseTime, duration)
    ...
    private func createBlockBuffer(with h264Format: H264Unit) -> CMBlockBuffer? {
        ...
        guard error == kCMBlockBufferNoErr else {
            Logger.shared.error("fail to create block buffer")
            return nil
        }
