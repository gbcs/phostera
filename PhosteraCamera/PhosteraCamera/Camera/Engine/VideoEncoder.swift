...
//  Phostera Camera
//
//  Created by Gary Barnett on 7/23/23.
//
...
class H264Encoder: NSObject {
    ...
    func configureCompressSession() throws {
        ...
        let propertyDictionary = [
            ...
            kVTCompressionPropertyKey_MaxKeyFrameInterval: 15,
        ...
        Logger.shared.info("VT Compressor is ready to use")
    }
    ...
    private var encodingOutputCallback: VTCompressionOutputCallback = { (outputCallbackRefCon: UnsafeMutableRawPointer?, _: UnsafeMutableRawPointer?, status: OSStatus, flags: VTEncodeInfoFlags, sampleBuffer: CMSampleBuffer?) in
        guard let sampleBuffer = sampleBuffer else {
            Logger.shared.error("encodingOutputCallback: nil buffer")
            return
        }
        guard let refcon: UnsafeMutableRawPointer = outputCallbackRefCon else {
            Logger.shared.error("encodingOutputCallback: nil pointer")
            return
        }
        guard status == noErr else {
            Logger.shared.error("encodingOutputCallback: encoding failed")
            return
        }
        guard CMSampleBufferDataIsReady(sampleBuffer) else {
            Logger.shared.error("encodingOutputCallback: CMSampleBuffer is not ready to use")
            return
        }
        guard flags != VTEncodeInfoFlags.frameDropped else {
            Logger.shared.error("encodingOutputCallback: frame dropped")
            return
        }
   ...
   //Set outgoing timestamps
    
    var baseTime = CMTime(value: 0, timescale: 30)
    var baseDuration = CMTime(value: 1, timescale: 30)
    private func encode(buffer: CMSampleBuffer) {
        ...
        baseTime = CMTimeAdd(baseTime, baseDuration)
