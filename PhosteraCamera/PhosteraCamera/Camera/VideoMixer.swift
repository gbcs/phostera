...
class VideoMixer {
    ...
    func mix(videoPixelBuffer: CVPixelBuffer, depthPixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
        ...
        guard let outputPixelBuffer = newPixelBuffer else {
            Logger.shared.error("Allocation failure: Could not get pixel buffer from pool (\(self.description))")
            return nil
        }
        ...
        guard let fullRangeVertexBuffer = fullRangeVertexBuffer else {
            Logger.shared.error("Failed to create Metal vertex buffer")
            CVMetalTextureCacheFlush(textureCache!, 0)
            return nil
        }
        
        guard let sampler = sampler else {
            Logger.shared.error("Failed to create Metal sampler")
            CVMetalTextureCacheFlush(textureCache!, 0)
            return nil
        }
        ...
        guard let commandQueue = commandQueue,
            ...
            let commandEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: renderPassDescriptor) else {
                Logger.shared.error("Failed to create Metal command queue")
                CVMetalTextureCacheFlush(textureCache!, 0)
                return nil
        }
    ...
    func makeTextureFromCVPixelBuffer(pixelBuffer: CVPixelBuffer) -> MTLTexture? {
        ...
        guard let cvTexture = cvTextureOut, let texture = CVMetalTextureGetTexture(cvTexture) else {
            Logger.shared.error("Video mixer failed to create preview texture")
